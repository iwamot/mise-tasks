...
| Task | Purpose |
|------|---------|
| `gha-lint` | Run a GitHub Actions lint suite (actionlint, ghalint, pinact, zizmor) over workflows and composite actions (`action.yml` anywhere in the repo). Fixers run before checkers. actionlint is the [kjanat/actionlint](https://github.com/kjanat/actionlint) fork, which accepts the `$/` self-repository syntax that upstream 1.7.12 rejects. |
